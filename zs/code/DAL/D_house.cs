...
namespace DAL
{
	...
	public partial class D_house
	{
		...
		/// 是否存在该记录
		/// </summary>
		public bool Exists(tb_house model)
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select count(1) from tb_house");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(tb_house model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tb_house(");
			strSql.Append("id,housename,administrative,provinceid,cityid,countyid,address,propertycompany,propertycompanyintro,traffic,customer,longitude,latitude,isdel,sortnum,addtime)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(tb_house model)
		{
			StringBuilder strSql=new StringBuilder();
			StringBuilder setSql=new StringBuilder();
			strSql.Append("update tb_house set ");
			if(!String.IsNullOrEmpty(model.housename))
			{
				setSql.Append( "housename=@housename");
			}
		...
		/// 删除一条数据
		/// </summary>
		public bool Delete(tb_house model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_house ");
			strSql.Append(" where id=@id " );
			using (IDbConnection conn = DapperHelper.OpenConnection())
			{
				int count = conn.Execute(strSql.ToString(), model);
		...
		/// 获得数据列表
		/// </summary>
		public List<tb_house> GetList(tb_house model, ref int total)
		{
			List<tb_house> list;
			StringBuilder strSql=new StringBuilder();
			StringBuilder whereSql = new StringBuilder(" where 1 = 1 ");
			strSql.Append(" select  ROW_NUMBER() OVER(ORDER BY id desc) AS RID, * from tb_house ");
			...
			if(!String.IsNullOrEmpty(model.housename))
			{
				whereSql.Append( " and housename=@housename");
			}
			if(!String.IsNullOrEmpty(model.administrative))
			{
			...
			using (IDbConnection conn = DapperHelper.OpenConnection())
			{
				list = conn.Query <tb_house>(pageSqlStr, model)?.ToList();
