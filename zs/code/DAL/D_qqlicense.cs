...
namespace DAL
{
	...
	/// 数据访问类:D_qqlicense
	/// </summary>
	public partial class D_qqlicense
	{
		public D_qqlicense()
		{}
		#region  Method
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(tb_qqlicense model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 获得数据列表
		/// </summary>
		public List<tb_qqlicense> GetList(tb_qqlicense model, ref int total)
		{
			List<tb_qqlicense> list;
			StringBuilder strSql=new StringBuilder();
			StringBuilder whereSql = new StringBuilder(" where 1 = 1 ");
			strSql.Append(" select  ROW_NUMBER() OVER(ORDER BY  desc) AS RID, * from tb_qqlicense ");
			if(!String.IsNullOrEmpty(model.id))
			{
			...
			using (IDbConnection conn = DapperHelper.OpenConnection())
			{
				list = conn.Query <tb_qqlicense>(pageSqlStr, model)?.ToList();
