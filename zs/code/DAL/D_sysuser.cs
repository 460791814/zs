...
namespace DAL
{
	...
	/// 数据访问类:D_sysuser
	/// </summary>
	public partial class D_sysuser
	{
		public D_sysuser()
		{}
		#region  Method
		...
		/// 是否存在该记录
		/// </summary>
		public bool Exists(tb_sysuser model)
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select count(1) from tb_sysuser");
		...
		/// 删除一条数据
		/// </summary>
		public bool Delete(tb_sysuser model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_sysuser ");
			strSql.Append(" where id=@id " );
			using (IDbConnection conn = DapperHelper.OpenConnection())
			{
				int count = conn.Execute(strSql.ToString(), model);
		...
		/// 获得数据列表
		/// </summary>
		public List<tb_sysuser> GetList(tb_sysuser model, ref int total)
		{
			List<tb_sysuser> list;
			StringBuilder strSql=new StringBuilder();
			StringBuilder whereSql = new StringBuilder(" where 1 = 1 ");
			strSql.Append(" select  ROW_NUMBER() OVER(ORDER BY id desc) AS RID, * from tb_sysuser ");
			...
			if(!String.IsNullOrEmpty(model.password))
			{
				whereSql.Append( " and password=@password");
			}
			...
			using (IDbConnection conn = DapperHelper.OpenConnection())
			{
				list = conn.Query <tb_sysuser>(pageSqlStr, model)?.ToList();
