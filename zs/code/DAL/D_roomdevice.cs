...
namespace DAL
{
	...
	public partial class D_roomdevice
	{
		...
		/// 得到一个对象实体
		/// </summary>
		public tb_roomdevice GetInfo(tb_roomdevice model)
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select * from tb_roomdevice");
			strSql.Append("  where id=@id ");
			using (IDbConnection conn = DapperHelper.OpenConnection())
			{
				model = conn.Query <tb_roomdevice>(strSql.ToString(), model)?.FirstOrDefault();
			}
			return model;
		}
		#endregion  Method
